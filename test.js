...
const LINES = 8;
let totalRTP = 0;
const wildWeight = PAYTABLE.find(s => s.id === 'wild').weight;
...
PAYTABLE.forEach(symbol => {
  if (symbol.id === 'scatter') {
    ...
    const expected = probLine * symbol.payout * LINES;
    totalRTP += expected;
    if (symbol.id === 'five') console.log('five RTP:', expected * 100, '%');
  }
});
console.log('RTP:', (totalRTP * 100).toFixed(2) + '%');
