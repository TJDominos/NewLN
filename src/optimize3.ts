...
// To lower RTP, we need to make the distribution MORE uniform.
// We can do this by raising the weights to a power < 1.
let bestPower = 1.0;
let minDiff = 100;

for (let p = 0.5; p <= 1.5; p += 0.001) {
  let tempPT = JSON.parse(JSON.stringify(PAYTABLE));
  tempPT.forEach((s: any) => {
    if (!['five'].includes(s.id)) {
      s.weight = Math.pow(s.weight, p);
    }
  });
...
console.log('Best Power:', bestPower);
PAYTABLE.forEach(s => {
  if (!['five'].includes(s.id)) {
    s.weight = parseFloat(Math.pow(s.weight, bestPower).toFixed(2));
  }
});
